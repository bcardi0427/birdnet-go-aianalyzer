...
- Fixed empty or corrupted ONNX Runtime caches breaking release builds by removing caching for ONNX Runtime altogether in [.github/actions/setup-onnxruntime/action.yml](file:///F:/AntiGravity%20Sources/birdnet-go/.github/actions/setup-onnxruntime/action.yml) (as downloading the small library is extremely fast and completely bypasses GitHub Actions caching bugs on macOS).
